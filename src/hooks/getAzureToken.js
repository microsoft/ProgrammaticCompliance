...
const checkGroupMembership = async (accessToken) => {
  try {
    ...
    const response = await fetch(`https://graph.microsoft.com/v1.0/me/memberOf?$filter=id eq \'${groupId}\'`, {
      headers: {
    ...
    if (data.error) {
      window.location.href = 'https://programmatic-compliance-gh1.azurewebsites.net/unauthorized';
